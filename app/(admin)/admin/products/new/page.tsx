...
export default async function AddProductPage() {
  ...
  async function addProduct(formData: FormData) {
    ...
    await prisma.product.create({
      data: { name, slug, description, price, stock, categoryId, images: [] }
    });
  ...
  return (
    <Card className="max-w-3xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>Điền thông tin chi tiết cho sản phẩm của bạn.</CardDescription>
      </CardHeader>
      <CardContent>
        <form action={addProduct} className="space-y-6">
          <div className="space-y-2">
            <Label htmlFor="name">Tên sản phẩm</Label>
            <Input id="name" name="name" placeholder="Ví dụ: Áo Thun Cotton" required />
          </div>

          <div className="space-y-2">
            <Label htmlFor="description">Mô tả</Label>
            <Textarea id="description" name="description" placeholder="Mô tả chi tiết về sản phẩm..." />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label htmlFor="price">Giá</Label>
              <Input id="price" name="price" type="number" placeholder="Ví dụ: 250000" required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="stock">Tồn kho</Label>
              <Input id="stock" name="stock" type="number" placeholder="Ví dụ: 100" required />
            </div>
          </div>
